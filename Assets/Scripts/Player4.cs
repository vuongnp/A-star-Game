...
public class Player4 : MovingObject
{ 
    private Rigidbody2D r2;
    private Animator animator;           
    private bool flag = true; // kiểm tra va chạm với Fire
    bool flag2 = false; // kiểm tra khi đang di chuyển
    ...
    protected override void Start()
    {
        ...
        r2.gravityScale = 0;
        r2.rotation = 0;
		ScoreScipt4.scoreValue4=0;  //điểm của Bot3		
        ...
        astar = new AStarSearch3(graph, start, goal, costObject);
        path = astar.FindPath();      
    ...
    int i = 0;
    //hàm di chuyển đến đích theo đường đi tìm được
    private void Tick()
    {
    ...
    {     
        if (other.tag == "Axe")
        {
			ScoreScipt4.scoreValue4+=20;
            other.gameObject.SetActive(false);
            animator.SetTrigger("player4Ready");
            flag = false;
        }
        else if (other.tag == "Fire")
        {
            if (ScoreScipt4.scoreValue4 != 0)
                ScoreScipt4.scoreValue4 -= 10;
        }
